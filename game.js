...
(function () {
  ...
  const DEBUG_PUZZLES = [
    ...
    },
    {
      ...
      bugIndex: 3,
      explanation: 'PROCESS happens directly after EXTRACT with no VALIDATE step. LLM outputs are probabilistic — they need schema validation against expected types, value ranges, and reference data before being trusted. Fix: insert VALIDATE between EXTRACT and PROCESS.',
      hint: 'Can you trust an AI output without checking it?',
      options: [1, 2, 3, 4],
    ...
    },
    {
      ...
      bugIndex: 5,
      explanation: 'END comes after LOG with no CLEANUP step. On bot exit — normal or crashed — the SAP session stays open until SAP own timeout. Fix: add CLEANUP before END to explicitly call session.Close(). This must run even on exception paths.',
    ...
    },
    {
      ...
      scenario: 'Ops receives ~4,000 notification emails overnight. The bot should send one summary email at the end, but fires notifications constantly.',
      flow: ['START', 'FETCH', 'LOOP', 'QUEUE_GET', 'VALIDATE', 'PROCESS', 'NOTIFY', 'QUEUE_SET', 'LOG', 'END'],
      bugIndex: 6,
      explanation: 'NOTIFY is inside the LOOP body — it fires on every single queue item. In a batch of 200 invoices that is 200 emails. Fix: move NOTIFY after the loop ends so it sends one summary notification.',
    ...
    },
    {
      ...
      scenario: 'Bot is stuck retrying the same invoice for 6 hours. The invoice has an invalid vendor code — a permanent data problem, not a transient error.',
      flow: ['START', 'FETCH', 'LOOP', 'QUEUE_GET', 'VALIDATE', 'RETRY', 'PROCESS', 'QUEUE_SET', 'LOG', 'END'],
      bugIndex: 5,
      explanation: 'RETRY wraps PROCESS indiscriminately — it retries both system exceptions and business exceptions. Business exceptions are permanent and should not be retried. Fix: add a DECISION after VALIDATE to separate business exceptions from system exceptions.',
    ...
    },
    {
      ...
      scenario: 'Customer enrichment bot runs 3 API calls in parallel. Results merge correctly, but ~20% of records show yesterday credit score instead of today.',
      flow: ['START', 'FETCH', 'PARALLEL', 'PROCESS', 'LOG', 'END'],
      bugIndex: 3,
      explanation: 'PROCESS merges parallel results immediately after PARALLEL with no TIMEOUT guard. One branch sometimes takes 15 seconds and returns cached data. Fix: add TIMEOUT after PARALLEL to enforce a deadline, then VALIDATE before PROCESS to check data freshness.',
    ...
    },
    {
      ...
      bugIndex: 5,
      explanation: 'RELEASE only executes in the happy path — after PROCESS succeeds. If VALIDATE or PROCESS throws an exception, the flow jumps to END without releasing the lock. Fix: RELEASE must run in a Finally block that executes regardless of success or failure.',
    ...
    },
    {
      ...
      bugIndex: 0,
      explanation: 'START goes directly into LOOP without a FETCH step to configure queue sort order. The bot pulls items in default FIFO order, ignoring Priority and Deadline fields. Fix: add FETCH before LOOP to configure the queue filter and sort.',
    ...
    },
    {
      ...
      bugIndex: 3,
      explanation: 'PROCESS runs directly without QUEUE_GET inside the loop. In Dev, FETCH loaded test data into a local variable. In Prod the loop has no data source and exits cleanly with zero items processed. Fix: add QUEUE_GET inside the loop.',
    ...
    },
    {
      ...
      scenario: 'Support ticket triage bot deployed 2 weeks ago. 85% of tickets go to billing — including password resets, delivery tracking, product questions.',
      flow: ['START', 'FETCH', 'AGENT', 'PROCESS', 'NOTIFY', 'LOG', 'END'],
      bugIndex: 2,
      explanation: 'AI AGENT classifies the ticket correctly, but there is no DECISION gateway after it to branch the flow. PROCESS always runs the same default billing path — the classification output is computed but never used. Fix: add DECISION after AGENT to route based on classification.',
